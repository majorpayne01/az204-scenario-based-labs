...
## 4. Deploy Azure Functions and Web App
...
>**NOTE: IF YOU ARE USING THE DEMO DEPLOYMENT, SKIP THIS TASK - CONTINUE WITH [TASK 3](#task-3-open-solution)**
...
**Other CosmosDB Processing Function App**
<img width="955" height="589" alt="image" src="https://github.com/user-attachments/assets/c7d91c1c-ca0a-4ab1-9e72-ee5601e03ac7" />
...
**Other Stream Function App Environment Variables**
<img width="1151" height="546" alt="image" src="https://github.com/user-attachments/assets/95be717d-8295-4cf9-b2f0-661f516d24f8" />

16. Select **Save** to apply your changes. Select **Continue** when prompted.
...
Open File Explorer and navigate to the location where you saved the code earlier in the "Before the hands-on lab" exercise. Select the appropriate folder to continue in:

- Navigate into the **Starter** folder to use a Visual Studio solution which is missing several code sections that you will add in Task 4, next. This choice is appropriate for the hands-on lab, so that you are immersed in code that interacts with the Cosmos DB SDK.
- Navigate into the **Solution** folder to use a Visual Studio solution that already contains all the missing code. This choice is appropriate for the demo deployment, so that you can get a finished environment in place as quickly as possible.

> Select and navigate into either the **Starter** folder OR the **Solution** folder before continuing. Both solutions are identical apart from the missing code you will add in Task 4 if you choose **Starter**.

In the folder you navigated into, open the Visual Studio solution file: **CosmosDbIoTScenario.sln**.
...
After opening the solution, observe the included projects in the **Solution Explorer**:
...
Right-click on the `CosmosDbIoTScenario` solution in Solution Explorer, then select **Restore NuGet Packages**. (The packages may have already been restored upon opening the solution.)

### Task 4: Code completion and walk-through

>**NOTE: IF YOU NAVIGATED INTO THE _Solution_ FOLDER IN TASK 3, SKIP TASK 4 (THIS TASK) AND CONTINUE WITH [TASK 5](#task-5-deploy-cosmos-db-processing-function-app). ONLY DO TASK 4 (THIS TASK) IF YOU NAVIGATED INTO THE _Starter_ FOLDER IN TASK 3.**
