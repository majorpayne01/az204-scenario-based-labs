...
- Must upgrade the starter and solution project to **.NET 8** from **.NET 3**, which is no longer supported. This also includes upgrading the Newtonsoft.Json to 13.0.3 and making sure the **Microsoft.NET.SDK.Functions** version in your .csproj file of each solution is **4.2.0** or later as well as other package references. Once doing so, you will also need to update the function app code as specific services as the dependency injection was out of date.
- For the function and web app deployments I preferred publishing the web app locally, zipping it (dotnet clean -> dotnet build -> dotnet publish -> zip /$appName.zip -> az webapp deploy...) , then deploying it to azure so I didn't follow the instructions for those tasks.
- Additional environment variables for the function applications. More defaults are needed to be set in order for the function app to work. 
