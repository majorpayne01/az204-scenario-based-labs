...
- Will not be able to stream data via a stream analytics job directly to Power BI. This is only available on commercial and is being depreciated in October 2027 (At this point everyone would be using Microsoft Fabric). Because real time data is imperative with IoT, I replaced the Power BI Output from the stream analytics job with an Azure Function + SignalR.
